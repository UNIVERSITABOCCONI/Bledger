...
const headers = [
  { key: 'name', name: 'You’re an ADMIN in the following networks:', width: '55%', type: 'name' },
  { key: 'level', name: 'Members', width: '20%', type: 'level' },
  { key: 'status', name: 'Scope 1 & 2', width: '25%', type: 'status' },
]

const slice = computed(() => networksStore.admin || { content: [], total: 0, page: 0, size: 10 })
...
async function fetchPage({ page, size }: { page: number; size?: number }) {
  await networksStore.fetchAdminNetworks({
    page,
